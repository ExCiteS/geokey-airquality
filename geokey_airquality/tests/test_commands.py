...
class CheckMeasurementsTest(TestCase):
    ...
    def test_check_measurements(self):
        ...
        user_1 = UserF.create(**{'is_superuser': False})
        user_2 = UserF.create(**{'is_superuser': False})

        long_time_ago = timezone.now() - timedelta(60)

        location_1 = AirQualityLocationF.create(
            creator=user_1,
            created=long_time_ago
        )
        location_2 = AirQualityLocationF.create(
            creator=user_1,
            created=long_time_ago
        )
        location_3 = AirQualityLocationF.create(
            creator=user_2,
            created=long_time_ago
        )
        location_4 = AirQualityLocationF.create(
            creator=user_2,
            created=long_time_ago
        )
